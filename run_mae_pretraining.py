...
from my_datasets import build_pretraining_dataset
from engine_for_pretraining import train_one_epoch, UncertaintyWeighter
...
def get_args():
    ...
    parser.add_argument('--mask_ratio', default=0.9, type=float, help='ratio of the visual tokens/patches need be masked')
    parser.add_argument('--input_size', default=224, type=int, help='videos input size for backbone')
    parser.add_argument('--drop_path', type=float, default=0.0, metavar='PCT', help='Drop path rate (default: 0.1)')       
    parser.add_argument('--no_normlize_target', action='store_false', dest='normlize_target')
    parser.set_defaults(normlize_target=True)
    parser.add_argument('--use_st_block', default=False,  action='store_true', help='additional framewise attention block in VIT-Block') # model creation input
    parser.add_argument('--add_intra_attention', default=False,  action='store_true', help='additional framewise attention module in VIT-Attention') # model creation input
    
    #---------MiRA: FMP---------------------------------------------------------------------------------------    
    parser.add_argument('--add_fmp_attention', default=False,  action='store_true')     # model creation input 
    ...
    parser.add_argument('--color_jitter', type=float, default=0.0, metavar='PCT', help='Color jitter factor (default: 0.4)')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='./datasets/dataSpecCSV_combined/vox-celeb-dataset_video_specs_updated_only_inliers.csv', type=str, help='dataset path')
    ...
    parser.add_argument('--use_pretrained_checkpoint', default='', help='resume from global pretrain checkpoint')
...
def find_uw_group(optimizer, uw):
    idx = None
    for i, g in enumerate(optimizer.param_groups):
        if any(id(p) == id(uw.log_sigma) for p in g['params']):
            idx = i
            print(f"[UW] group_idx={i}  lr={g.get('lr')}  wd={g.get('weight_decay')}  lr_scale={g.get('lr_scale')}")
            break
    if idx is None:
        print("[UW] NOT found in optimizer param_groups!")
    return idx
...
def main(args):
    ...
    print("=== DEBUG TRAIN SETUP ===")
    print(args)
    print("Effective LR =", args.lr)
    print("Min LR =", args.min_lr)
    print("Warmup LR =", args.warmup_lr)
    ...
    # DEBUG
    for i, g in enumerate(optimizer.param_groups):
        print(f"param_group {i}: lr={g['lr']}, lr_scale={g.get('lr_scale')}, weight_decay={g.get('weight_decay')}")
    ...
    # DEBUG scheduler
    print("lr_schedule[0] =", lr_schedule_values[0])
    print("lr_schedule[1] =", lr_schedule_values[1])
    print("lr_schedule[10] =", lr_schedule_values[10])
    print("lr_schedule[end] =", lr_schedule_values[-1])

    uw = UncertaintyWeighter(task_names=['feature', 'recon'], init_log_sigma=0.0, clamp=(-3, 3), reg=1e-4).to(device)
    uw = None

    if len(args.use_pretrained_checkpoint) != 0:
        utils.load_pretrained_model(
            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler)
        if uw is not None:
            utils._attach_uw_param_group(optimizer, uw, optimizer.param_groups[0]['lr'])
    else:
        utils.auto_load_model(
            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler, uw=uw)

    if uw is not None:
        _ = find_uw_group(optimizer, uw)
        ...
        if args.output_dir:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                utils.save_model(
                    ...
                    loss_scaler=loss_scaler, epoch=epoch, uw=uw)
